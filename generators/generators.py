        ...
        with torch.no_grad():
            ...
            for b in range(batch_size):
                ...
                while head < transformed_points.shape[1]:
                    ...
                    coarse_output[
                        b:b + 1, head:
                        tail] = self.siren.forward_with_frequencies_phase_shifts(
                ...
                with torch.no_grad():
                    ...
                    fine_points = transformed_ray_origins.unsqueeze(
                        2).contiguous() + transformed_ray_directions.unsqueeze(
                            2).contiguous() * fine_z_vals.expand(
                                -1, -1, -1, 3).contiguous()
                    fine_points = fine_points.reshape(
                        batch_size, img_size * img_size * num_steps, 3)
                    #### end new importance sampling
                    ...
                    while head < fine_points.shape[1]:
                        ...
                        fine_output[
                            ...
                            tail] = self.siren.forward_with_frequencies_phase_shifts(
                                ...
                                truncated_phase_shifts[b:b + 1],
                                ray_directions=
                                transformed_ray_directions_expanded[b:b + 1,
                                                                    head:tail])
                        head += max_batch_size
            ...
            if hierarchical_sample:
                with torch.no_grad():
                    ...
                    fine_z_vals = sample_pdf(
                        z_vals_mid,
                        weights[:, 1:-1], num_steps, det=False).detach().to(
                            self.device
        ...
        if hierarchical_sample:
            with torch.no_grad():
                ...
                fine_points = transformed_ray_origins.unsqueeze(2).contiguous(
                    ...
                    -1, -1, -1, 3).contiguous()  # dimensions here not matching
                fine_points = fine_points.reshape(
                    batch_size, img_size * img_size * num_steps, 3)
                #### end new importance sampling
