...
class TALLSIREN(nn.Module):
    ...
    def forward_with_frequencies_phase_shifts(self, input, frequencies,
                                              phase_shifts, ray_directions,
                                              **kwargs):
        ...
        for index, layer in enumerate(self.network):
            ...
            x = layer(x, frequencies[..., start:end], phase_shifts[...,
                                                                   start:end])
...
class SPATIALSIRENBASELINE(nn.Module):
    """Same architecture as TALLSIREN but adds a UniformBoxWarp to map
    input points to -1, 1"""
...
def sample_from_3dgrid(coordinates, grid):
    ...
    Returns sampled features of shape (batch_size, num_points_per_batch,
                                       feature_channels)
    """
...
class EmbeddingPiGAN128(nn.Module):
    """Smaller architecture that has an additional cube of embeddings.
       Often gives better fine details."""
    ...
    def __init__(self,
        ...
        self.spatial_embeddings = nn.Parameter(
            torch.randn(1, 32, 96, 96, 96) * 0.01)

        # !! Important !! Set this value to the expected side-length of your
        # scene. e.g. for for faces, heads usually fit in
